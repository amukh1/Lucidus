...
antlrcpp::Any Codegen::visitDec(LucidusParser::DecContext *ctx) {
...
antlrcpp::Any Codegen::visitExpr(LucidusParser::ExprContext *ctx) {
    if(ctx->INT() != nullptr && ctx->children.size() == 1) {
        ...
        llvm::Type* T = getTypes(ctx->type(), this->controller, this->structs);
        llvm::Type* type;

        if (llvm::isa<llvm::PointerType>(T)) {
            // If T is a pointer type, get the actual type it points to
            llvm::Type* elementType = llvm::cast<llvm::PointerType>(T)->getElementType();
            if (llvm::isa<llvm::StructType>(elementType)) {
            // Now elementType is the actual struct type
            type = elementType;
            } else {
                type = T;
            }
        ...
        */
        // where T is the type
        // get_size function is incorrect

        llvm::Value* nullPtr = llvm::Constant::getNullValue(llvm::PointerType::getUnqual(type));
        llvm::Value* gep = controller->builder->CreateGEP(type, nullPtr, controller->builder->getInt32(1));
        llvm::Value* sizeI = controller->builder->CreatePtrToInt(gep, controller->builder->getInt32Ty());
        return sizeI;
...
antlrcpp::Any Codegen::visitDef(LucidusParser::DefContext *ctx) {
...
antlrcpp::Any Codegen::visitFunc(LucidusParser::FuncContext *ctx)  {
...
antlrcpp::Any Codegen::visitStat(LucidusParser::StatContext *ctx) {
    ...
    if(ctx->expr() !=nullptr && ctx->children.size() == 1) {
        ...
        // // e_handler2.visitor = std::make_shared<Codegen>(this);
...
std::any Codegen::visitStruct(LucidusParser::StructContext *ctx) {
